...
function isLikelyTrustedApiUrl(apiUrl) {
  ...
  try {
    ...
    if (TRUSTED_API_HOSTS.has(hostname))
      return true;
    if (hostname.endsWith(".openai.com") || hostname.endsWith(".anthropic.com") || hostname.endsWith(".googleapis.com"))
      return true;
...
function getApiUrlRiskWarning(apiUrl) {
  const trimmed = (apiUrl || "").trim();
  if (!trimmed || isLikelyTrustedApiUrl(trimmed))
    return "";
...
`;
  ...
  async handleSend(text, selectedModel = "") {
    ...
    try {
      ...
      if (apiUrlWarning && !confirm(`${apiUrlWarning}
...
Do you want to test this endpoint anyway?`)) {
      ...
      new import_obsidian3.Setting(div).setName(`API URL`).setDesc(m.authMode === "codex_auth" ? "Codex auth usually uses https://chatgpt.com/backend-api/codex/responses" : "").addText((text) => text.setPlaceholder("https://api.example.com/v1").setValue(m.apiUrl).onChange(async (value) => {
        ...
        if (warning) {
          new import_obsidian3.Notice("Warning: custom API URL may expose your note content and API key to an untrusted service.", 8e3);
        }
        this.scheduleSave();
      }));
